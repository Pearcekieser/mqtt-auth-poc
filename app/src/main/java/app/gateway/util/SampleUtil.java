...
public class SampleUtil {
    ...
    public static KeyStorePasswordPair getKeyStorePasswordPair(final InputStream certificateStream, final InputStream privateKeyStream,
                                                               String keyAlgorithm) {
        if (certificateStream == null || privateKeyStream == null) {
            System.out.println("Certificate or private key file missing");
            return null;
        }
        System.out.println("Cert file:" +certificateStream + " Private key: "+ privateKeyStream);

        try {
            final PrivateKey privateKey = PrivateKeyReader.getPrivateKey(privateKeyStream, keyAlgorithm);
    ...
    private static List<Certificate> loadCertificatesFromStream(final InputStream inputStream) {
        try {
            final CertificateFactory certFactory = CertificateFactory.getInstance("X.509");
            return (List<Certificate>) certFactory.generateCertificates(inputStream);
